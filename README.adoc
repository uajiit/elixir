...
On Debian or Ubuntu, `utils/setup-local` prepares one project and configures
Apache without manually editing `000-default.conf`. Set only the parent
directory used for Elixir projects, then provide the project name and its
source Git repository:

----
export LXR_PROJ_DIR=$HOME/self_project/elixir-workspace
./utils/setup-local riot $HOME/self_project/RIOT
----

The installer uses `/usr/local/elixir` as the Elixir installation and
`$HOME/venv` as the Python virtual environment by default. Override these
...
It creates `<LXR_PROJ_DIR>/<project>/repo` and `data`, installs the Python
requirements, indexes the repository, generates an Apache virtual host,
enables the required modules, and restarts Apache. Pass `--skip-index` when
configuring Apache for a project that has already been indexed.
