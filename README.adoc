...
The source to index must be a Git repository with at least one tag. Verify it
before setup:

----
git -C /path/to/source tag --list
----

From the default Elixir installation, run the installer:

----
cd /usr/local/elixir
./utils/setup-local <project> /path/to/source
----

`LXR_PROJ_DIR` defaults to the value in an existing generated Apache site, or
to `$HOME/elixir-workspace` on the first run. Override it when needed:

----
export LXR_PROJ_DIR=/path/to/elixir-workspace
----

For example:

----
./utils/setup-local mboot $HOME/self_project/mboot
----

The installer creates `<LXR_PROJ_DIR>/<project>/repo` and `data`, installs the
Python requirements, indexes all tagged versions, configures Apache, and
restarts it. Open:

----
http://localhost/<project>/latest/source
----

It uses `/usr/local/elixir` as the Elixir installation and `$HOME/venv` as the
Python virtual environment by default. Override either location when needed:

----
ELIXIR_HOME=/path/to/elixir ELIXIR_VENV=/path/to/venv \
    ./utils/setup-local <project> <source-repository>
----

Pass `--skip-index` only when that project already has a generated database:

----
./utils/setup-local --skip-index <project> <source-repository>
----

Re-run the normal command after adding source tags. It refreshes the bare
repository and indexes only new data.

== Development Deployment

When developing in a separate checkout while Apache uses
`/usr/local/elixir`, deploy changes to `script.sh` and restart Apache:

----
elixir_source=$(git rev-parse --show-toplevel)
elixir_home=${ELIXIR_HOME:-/usr/local/elixir}
sudo install -m 755 "$elixir_source/script.sh" "$elixir_home/script.sh"
sudo systemctl restart apache2
----

This deployment step is unnecessary when the development checkout itself is
the configured `ELIXIR_HOME`.

== Manual Installation

The following sections describe the individual installation steps for
environments where `utils/setup-local` cannot be used.
